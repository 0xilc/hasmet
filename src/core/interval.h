#pragma once
#include <iostream>
#include <cmath>

namespace hasmet {
class Interval {
 public:
  float min, max;

  Interval() : min(INFINITY), max(-INFINITY) {};
  Interval(float _min, float _max) : min(_min), max(_max) {}
  Interval(Interval _i0, Interval _i1)
      : min(std::min(_i0.min, _i1.min)), max(std::max(_i0.max, _i1.max)) {}

  const void thicken() {
    min = min - 0.0001f;
    max = max + 0.0001f;
  }

  inline Interval merge(const Interval& _other) const {
    return Interval(std::min(min, _other.min), std::max(max, _other.max));
  }

  inline bool overlap(const Interval& _other) const {
    return (min <= _other.max && _other.min <= max);
  }
  inline bool consists(const float& point) const {
    return (min <= point && max >= point);
  }
  inline float getLength() const { return max - min; }
};
} // namespace hasmet