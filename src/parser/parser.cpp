...
void parsePlyFile(const std::string& ply_filename, Mesh_& mesh, Scene_& scene) {
  ...
  for (long i = 0; i < num_faces; ++i) {
    ...
    if (format == ASCII) {
      ...
      if (!file) {
        std::cerr << "Error: Unexpected end of file reading face count from "
                  << ply_filename << std::endl;
        return;
      }
      num_verts_in_face = extract_int_from_buffer(
          count_buffer.data(), face_prop.count_type, file_is_little_endian);

      for (int j = 0; j < num_verts_in_face; ++j) {
        file.read(index_buffer.data(), face_prop.index_size);
        if (!file) {
          std::cerr << "Error: Unexpected end of file reading face index from "
                    << ply_filename << std::endl;
          return;
        }
        local_indices.push_back(extract_int_from_buffer(
            index_buffer.data(), face_prop.index_type, file_is_little_endian));
      }
    }

    // --- Triangulation (same logic for all formats) ---
    if (num_verts_in_face < 3) {
      std::cerr << "Warning: Skipping face " << i
                << " (less than 3 vertices) in " << ply_filename << std::endl;
      continue;
    }
...
void parseScene(const std::string& filename, Scene_& scene) {
  ...
  if (objects_json.contains("MeshInstance")) {
    ...
    auto parse_mesh_instance = [&](const json& mesh_instance_json) {
      ...
      inst.base_mesh_id =
          std::stoi(mesh_instance_json["_baseMeshId"].get<std::string>());
  ...
  if (objects_json.contains("Triangle")) {
    ...
    auto parse_triangle = [&](const json& tri_json) {
      ...
      if (tri_json.contains("Transformations")) {
        parse_transform_refs(tri_json["Transformations"].get<std::string>(),
                             tri.transformations);
      }
  ...
  if (objects_json.contains("Sphere")) {
    ...
    auto parse_sphere = [&](const json& sphere_json) {
      ...
      sphere.material_id =
          std::stoi(sphere_json["Material"].get<std::string>());
  ...
  if (objects_json.contains("Plane")) {
    ...
    auto parse_plane = [&](const json& plane_json) {
      ...
      plane.point_vertex_id =
          std::stoi(plane_json["Point"].get<std::string>()) - 1;
      plane.normal = parseVec3f(plane_json["Normal"]);
      if (plane_json.contains("Transformations")) {
        parse_transform_refs(plane_json["Transformations"].get<std::string>(),
...
void printScene(const Scene_& scene) {
  ...
  std::cout << "  Shadow Ray Epsilon: " << scene.shadow_ray_epsilon
            << std::endl;
  std::cout << "  Max Recursion Depth: " << scene.max_recursion_depth
            << std::endl;
  std::cout << " Intersection Test Epsilon" << scene.intersection_test_epsilon
            << std::endl;
  ...
  for (const auto& cam : scene.cameras) {
    ...
    std::cout << "    Resolution: " << cam.image_width << "x"
              << cam.image_height << std::endl;
  ...
  // --- Vertex Data ---
  /*std::cout << "\n[VERTEX DATA (" << scene.vertex_data.size() << " vertices)]"
  << std::endl;

  int vertices_to_print = (int)scene.vertex_data.size();
  for (int i = 0; i < vertices_to_print; ++i) {
      std::cout << "  Vertex " << i + 1 << ": " << scene.vertex_data[i] <<
  std::endl;
  }*/
  ...
  for (const auto& mesh : scene.meshes) {
    ...
    if (!mesh.faces.empty()) {
      ...
      std::cout << "    First Face Indices: " << face.v0_id << ", "
                << face.v1_id << ", " << face.v2_id << std::endl;
    }
    std::cout << "    Transformations:";
  ...
  for (const auto& tri : scene.triangles) {
    ...
    std::cout << "    Indices: " << tri.v0_id << ", " << tri.v1_id << ", "
              << tri.v2_id << std::endl;
    std::cout << "    Transformations:";
  ...
  for (const auto& sphere : scene.spheres) {
    ...
    std::cout << "    Center Vertex ID: " << sphere.center_vertex_id
              << std::endl;
